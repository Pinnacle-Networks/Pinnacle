...
def up_cluster(
    ...
    cmd = f"SUPERDUPERDB_CONFIG={CFG} {python_executable} -m pinnacledb vector-search"
    run_tmux_command(['send-keys', '-t', f'{SESSION_NAME}:vector-search', cmd, 'C-m'])
    if cdc:
        cmd = f"SUPERDUPERDB_CONFIG={CFG} {python_executable} -m pinnacledb cdc"
        run_tmux_command(['send-keys', '-t', f'{SESSION_NAME}:cdc', cmd, 'C-m'])
    cmd = f"SUPERDUPERDB_CONFIG={CFG} {python_executable} -m pinnacledb rest"
